...
# Import repo monitor for repository context
import sys
import os
...
class Field:
    ...
    def __init__(self):
        ...
        # Repository monitor for file changes
    ...
    def initialize_population(self):
        ...
        # Add repository context from repo_monitor
        try:
            repo_changes = self.repo_monitor.get_changed_files()
            if repo_changes:
                repo_context = " ".join([f"File {f}: {self.repo_monitor.get_file_content(f)[:200]}..." for f in list(repo_changes.keys())[:5]])
                context += " " + repo_context
        except Exception as e:
            log_metrics(f"Repo monitor error: {e}", "WARNING")
